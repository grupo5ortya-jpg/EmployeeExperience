...
/* ── Main form modal ──────────────────────────────────────────── */
export default function ExitInterviewForm({ assignment, onClose, onCompleted }) {
  const { survey, surveyId, employeeId } = assignment
  ...
  const handleSubmit = async () => {
    ...
    try {
      const responses = closedQuestions.map((q) => {
      ...
      await submitExitInterview({ surveyId, employeeId, responses })

      await queryClient.invalidateQueries({ queryKey: ['pending-exit-interviews', employeeId] })
      ...
      <div className="bg-white rounded-2xl border border-brand-light shadow-xl w-full max-w-xl">
        ...
        {done ? (
          <div className="flex flex-col items-center gap-4 px-6 py-12 text-center">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-800">¡Entrevista completada!</h3>
              <p className="text-sm text-slate-500 mt-1">Gracias por tu tiempo. Te deseamos lo mejor en tu nueva etapa.</p>
            </div>
            <button
              type="button"
              onClick={onCompleted}
