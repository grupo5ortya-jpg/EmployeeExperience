...
function SectionTitle({ children }) {
  return (
    <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider pt-2 pb-1
...
export default function CreateEmployeeModal({ isOpen, onClose, onSave, employees }) {
  ...
  // Derive unique departments from existing employees
  const depts = Object.values(
    employees.reduce((acc, e) => {
      if (e.department) acc[e.department.id] = e.department
      return acc
    }, {})
  )
  ...
  const managers = employees.map((e) => ({
    id:   e.id,
    name: `${e.firstName} ${e.lastName} — ${e.position ?? ''}`.trim(),
  }))
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-navy/40 backdrop-blur-sm overflow-y-auto py-6 px-4">
      <div className="bg-white rounded-2xl border border-brand-light shadow-xl w-full max-w-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-brand-light">
          <div>
            <h2 className="text-base font-bold text-slate-800">Nuevo colaborador</h2>
            <p className="text-xs text-slate-400 mt-0.5">Completá los datos para crear el perfil</p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Nombre" required>
                <input name="firstName" value={form.firstName} onChange={handle}
                  className={inputCls} placeholder="Ej. María" required />
              </Field>
              <Field label="Apellido" required>
                <input name="lastName" value={form.lastName} onChange={handle}
                  className={inputCls} placeholder="Ej. González" required />
              </Field>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <Field label="Número de documento" required>
                <input name="documentNumber" value={form.documentNumber} onChange={handle}
                  className={inputCls} placeholder="Ej. 30111222" required />
              </Field>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Posición / Cargo">
                <input name="position" value={form.position} onChange={handle}
                  className={inputCls} placeholder="Ej. Frontend Developer" />
              </Field>
              <Field label="Estado">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Teléfono">
                <input name="phone" value={form.phone} onChange={handle}
                  className={inputCls} placeholder="Ej. 1122334455" />
              </Field>
              <Field label="Dirección">
                <input name="address" value={form.address} onChange={handle}
                  className={inputCls} placeholder="Ej. Av. Corrientes 1234" />
              </Field>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Nombre">
                <input name="emergencyContactName" value={form.emergencyContactName} onChange={handle}
                  className={inputCls} placeholder="Ej. Juan González" />
              </Field>
              <Field label="Teléfono">
                <input name="emergencyContactPhone" value={form.emergencyContactPhone} onChange={handle}
                  className={inputCls} placeholder="Ej. 1199887766" />
              </Field>
            </div>
          </div>
